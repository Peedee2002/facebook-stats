...
fn main() {
    ...
    let mut chart = ChartBuilder::on(&root)
        ...
        .build_cartesian_2d(0..24, 0..2000)
        .expect("ahsa");
            ...
            Histogram::vertical(&chart)
                ...
                .data(data.clone().map(|x: (NaiveDateTime, String)| {
                    (
                        ...
                        if !x.1.starts_with("Peter") { 1 } else { 0 },
                    )
                })),
        )
        .expect("draw")
    ...
    chart
        .draw_series(
            Histogram::vertical(&chart)
                ...
                .data(
                    data.clone()
                        .map(|x: (NaiveDateTime, String)| (x.0.time().hour() as i32, 1)),
                )
                .baseline_func(|bar| {
                    data.clone()
                        .filter(|val| val.0.hour() as i32 == *bar && !val.1.starts_with("Peter"))
                        .count() as i32
                }),
        )
        .expect("draw")

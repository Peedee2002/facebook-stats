...
fn main() {
    ...
    let root = BitMapBackend::new("addiction.png", (640, 480)).into_drawing_area();
    root.fill(&WHITE).expect("cant draw?");
    let mut chart = ChartBuilder::on(&root)
        .caption("texting habits", ("sans-serif", 50).into_font())
        .margin(5)
    ...
    chart.draw_series(
        ...
        .data(data.clone().map(|x: (NaiveDateTime, String)| (x.0.time().hour() as i32, if !x.1.starts_with("Peter") { 1 } else { 0 } )))
    ).expect("draw")
    .label("Others")
    ...
    chart.draw_series(
        ...
        .data(data.clone().map(|x: (NaiveDateTime, String)| (x.0.time().hour() as i32, 1 )))
        .baseline_func(|bar| {
            data.clone().filter(|val| val.0.hour() as i32 == *bar && !val.1.starts_with("Peter")).count() as i32
        })
    ).expect("draw")
    .label("Peter")
